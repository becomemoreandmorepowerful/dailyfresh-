...
class OrderPlaceView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        # 组织上下文,传入json必须是字符串的形式,需要把列表转化成字符串
...
class OrderCommitView1(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            conn = get_redis_connection('default')
            cart_key = 'cart_%d'%user.id
            # 把从json中传递过来的字符串转化成列表。
            sku_ids = sku_ids.split(',')
            for sku_id in sku_ids:
                # 获取商品的信息
                try:
                    # select * from df_goods_sku where id=sku_id for update;  for update 相当有进程中的加锁，谁先取得锁谁先进行事务，其他人在此等待；
                    sku = GoodsSKU.objects.select_for_update().get(id=sku_id)
                except:
                    # 商品不存在 事务回滚， 不会在数据库中更新数据
        ...
        try:
            ...
            for sku_id in sku_ids:
                for i in range(3):
                    ...
                    # 返回受影响的行数   在原有的库存上进行更新，如果origin不是原有的查询出来的库存，即库存发生变化，则res=0,即下单失败，否则在库存没有变化的基础上进行更新库存和销量
...
class OrderPayView(View):
    ...
    def post(self,request):
        ...
        try:
            ...
            # 初始化
        alipay = AliPay(
            appid="2016091800536588",  # 应用id
            app_notify_url=None,  # 默认回调url
            app_private_key_path=os.path.join(settings.BASE_DIR, 'apps/order/app_private_key.pem'),
            alipay_public_key_path=os.path.join(settings.BASE_DIR, 'apps/order/alipay_public_key.pem'),

            sign_type="RSA2",  # RSA 或者 RSA2
            debug=True  # 默认False  False为真实的地址 True访问的是沙箱的地址
        )
        ...
        return JsonResponse({'res': 3, 'pay_url': pay_url,'message':'成功'})
...
class OrderCheckView(View):
    ...
    def post(self,request):
        ...
        alipay = AliPay(
            ...
            sign_type="RSA2",  # RSA 或者 RSA2
            debug=True  # 默认False  False为真实的地址 True是沙箱的地址
        )
        while True:
            # 支付宝查询交易接口
